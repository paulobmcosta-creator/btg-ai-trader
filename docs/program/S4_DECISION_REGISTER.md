...
| DD-69 | Especificação de targets, labels e horizontes de previsão/decisão | `CANONICAL_DEFERRED` — Canonical production targets remain deferred. Generic target specification is `TRIGGERED_AND_SATISFIED` locally via `TargetSemantics` (`CONTINUOUS`, `BINARY_PROBABILITY`, `CATEGORICAL`) with explicit `feature_knowledge_time` and `target_knowledge_time`. | Sample domain contract |
...
| DD-87 | Método de purging e horizonte de purging | `TRIGGERED_AND_SATISFIED` — Configurable information-horizon purging removes samples whose label availability crosses evaluation boundaries (`target_knowledge_time > eval_start`). Fail-closed purging enforced when horizon is unspecified. | Overlapping label handling |
| DD-88 | Método de embargo e duração de embargo | `TRIGGERED_AND_SATISFIED` — Configurable temporal embargo offsets evaluation or excludes post-training intervals to prevent serial autocorrelation. Duration is explicit and required. | Post-training separation |
...
| DD-91 | Thresholds numéricos de estabilidade entre folds | `PARTIALLY_TRIGGERED` — Factual stability diagnostics (worst fold, dispersion, range, sign consistency) implemented in `FoldStabilityDiagnostics`. Universal pass/fail stability thresholds remain deferred. | Fold stability analysis |
| DD-100 | Definição formal do estimand de cada estudo | `CANONICAL_DEFERRED`, `S4_LOCAL_ESTIMAND = EXPERIMENT_LOCAL` — Estimands are specified per evaluation run/boundary; universal study estimands remain deferred. | Study estimand contract |
| DD-101 | Lista de métricas primárias e secundárias do experimento | `PARTIALLY_TRIGGERED` — Standard metrics implemented in `metrics.py`; experiment-level primary/secondary metric designation remains experiment-local / deferred to Sprint 5. | Metric taxonomy |
| DD-102 | Níveis de significância estatística (alpha) e intervalos de confiança | `NOT_TRIGGERED_AND_DEFERRED` — S4 baselines use descriptive statistics only; inferential significance testing and p-values are strictly deferred to prevent unverified claims. | Inferential statistics |
| DD-103 | Escolha da metodologia inferencial (frequentista, bayesiana, bootstrap) | `NOT_TRIGGERED_AND_DEFERRED` — Inferential methodology is deferred; S4 provides deterministic descriptive evaluation. | Inference framework |
| DD-104 | Comprimento dos blocos em bootstrap temporal | `NOT_TRIGGERED_AND_DEFERRED` — Temporal block bootstrap is not implemented in S4 deterministic kernel. | Resampling methods |
| DD-105 | Métodos específicos de controle de multiplicidade | `PARTIALLY_TRIGGERED` — `SearchFamily` records all considered candidates and search history to prevent survivorship bias; formal family-wise error rate / FDR control algorithms remain deferred. | Multiplicity control |
| DD-108 | Grade de parâmetros para testes de sensibilidade e perturbação de custos | `NOT_TRIGGERED_AND_DEFERRED` — Cost perturbation belongs to financial backtesting in Sprint 3; statistical baselines evaluate predictive targets without execution costs. | Sensitivity testing |
| DD-111 | Limiares exatos de decisão e confiança da estratégia | `NOT_TRIGGERED_AND_DEFERRED` — Strategy operational decision thresholds belong to Sprint 7; S4 evaluates predictions without strategy execution. | Strategy thresholds |
| DD-112 | Regras de penalização de complexidade algorítmica | `NOT_TRIGGERED_AND_DEFERRED` — Formal complexity penalization (AIC, BIC, regularizers) for ML models deferred to Sprint 5. | Complexity control |
...
In alignment with Protocol 0E-C, data roles are strictly partitioned into `DEVELOPMENT`, `TRAINING_FIT`, `VALIDATION_SELECTION`, and `PROTECTED_TEST`. Protected test evidence cannot be used for parameter tuning, threshold optimization, or winner selection.
...
If historical observations are insufficient, baselines emit explicit `is_cold_start = True` with `fallback_value` or fail closed with `ValueError` if `fallback_value` is `None`. Silent imputation is forbidden.
...
Metrics and probabilities use Python `Decimal` with explicit rounding via `NumericPolicy` to guarantee cross-platform numerical stability and exact reproducibility.
...
Candidates can only be compared when evaluated against identical populations, boundaries, folds, targets, and metric semantics. Mismatched comparisons fail closed, raising `ParityViolationError`.

### S4-D-12 — Structural protection against protected test winner selection
Automated selection helpers reject winner selection when folds include `EvaluationRole.PROTECTED_TEST`, raising `ValueError`. Reusing protected evidence across adapted candidates raises `ProtectedEvidenceReuseError`.

### S4-D-13 — Search family multiplicity tracking
The evaluation manifest records all candidates, baselines, and configurations evaluated within a `SearchFamily` (`candidate_ids_considered`, `configurations_considered`, `selection_domain`, `selection_metric`, `baseline_ids`) to maintain scientific provenance.
