...
|---|---|---|---|
| DD-13 | Algoritmo concreto de RNG para simulação | Foundation DEFERRED; DECIDED FOR S3 LOCAL BASELINE — Baseline backtest kernel is strictly deterministic without pseudo-random number generation. RNG is absent and forbidden in baseline simulation. Formal operational RNG remains deferred by Foundation. | Deterministic backtest baseline |
| DD-14 | Algoritmo de inicialização, derivação e particionamento de seeds | Foundation DEFERRED; DECIDED FOR S3 LOCAL BASELINE — Deterministic execution and run derivation via SHA-256 / UUID5 hashes without PRNG seeds. Operational seed derivation remains deferred by Foundation. | Deterministic execution runs |
| DD-15 | Formato físico e representação do RunInputBoundary | DECIDED FOR S3 — Materialized as `BacktestInputBoundary` with mandatory explicit source provenance (`source_run_id`, `source_code_revision`, `source_config_hash`, `provider_id`, `capture_scope`). Fabricated fallback hashes (`'0'*64`) and dummy run IDs are strictly forbidden and rejected fail-closed. | First backtest run |
| DD-16 | Critérios de determinismo e replay | DECIDED FOR S3 — Strict byte-for-byte reproducibility across runs requiring valid replay schedule or explicit source boundary; AST-enforced exclusion of stochastic sources (`random`, `uuid4`, `secrets`, `os.urandom`, etc.) including alias imports. | Backtest boundary checker |
| DD-46 | Metodologia de cost basis (custo médio ponderado vs FIFO) | Foundation DEFERRED; DECIDED FOR S3 LOCAL SIMULATION — Moving weighted average cost basis using exact `Decimal` arithmetic for local backtest accounting. Canonical production ledger cost basis remains deferred by Foundation. | Position accounting |
| DD-47 | Metodologia de P&L e Valuation intradiário (Mark-to-Market) | Foundation DEFERRED; DECIDED FOR S3 LOCAL SIMULATION — Intraday mark-to-market valuation strictly requires contemporary valid `BacktestMarkEvidence` and fails closed (`unrealized_pnl=None`, `total_net_pnl=None`) when mark quotes are absent. Canonical production valuation remains deferred by Foundation. | Intraday MTM valuation |
| DD-74 | Modelagem paramétrica de custos de transação e fricções | Foundation DEFERRED; DECIDED FOR S3 LOCAL SIMULATION — Parametric modeling for local simulation via explicit `InstrumentEconomics`, `FeeSchedule`, adverse deterministic slippage models, and diagnostic spread burden. Canonical production friction tables remain deferred by Foundation. | Backtest economic models |
| DD-92 | Resolução temporal de simulação de backtest (ticks vs trades vs candles) | DECIDED FOR S3 (PARTIAL) — High-resolution tick simulation supported; candle execution path is deferred and fails closed (`INDETERMINATE` with `CANDLE_EXECUTION_PATH_UNSUPPORTED`). | Market data resolution |
| DD-93 | Tabela de taxas de negociação, registro e emolumentos B3 | DECIDED FOR S3 LOCAL SIMULATION ONLY — Parametric local `FeeSchedule(fixed_per_order, per_unit, bps_rate, currency)` with temporal validity (`effective_from`/`effective_until`). Definitive canonical B3 tariff/emolument table in production remains deferred. | Transaction cost schedule |
...
| DD-98 | Regras de liquidação mandatória de posições no fim do pregão simulado | DECIDED FOR S3 (PARTIAL) — Default policy `KEEP_OPEN` supported; mandatory liquidation policy `CLOSE_AT_LAST_VALID_QUOTE` is deferred and raises `NotImplementedError`. | End-of-window policy |
| DD-99 | Biblioteca ou engine físico de simulação causal de backtest | DECIDED FOR S3 LOCAL BASELINE — In-house deterministic execution economics kernel materialized in `btg_ai_trader.backtesting`. | Backtest engine architecture |
