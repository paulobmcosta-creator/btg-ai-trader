...
8. **Test Coverage:** `tests/replay/test_causal_replay_core.py` (31 tests) and `tests/test_s2_boundary.py` (8 tests) covering all positive and negative replay requirements with 100% line and branch coverage on replay core.
...
3. **Lossless Preservation:** The caller iterable is defensively materialized into an immutable tuple. The accepted immutable `EventEnvelope` instances are preserved by identity and are neither mutated nor reconstructed, retaining all source facts, identity, temporal fields, Tick and Candle payloads, and envelope/schema versions without alteration.
...
7. **Test Coverage:** `tests/data_platform/test_normalization_quality.py` (24 tests) validating lossless preservation, explicit missingness, quality finding traceability, boundary error handling, and immutability.

---

## 5. S2-AC-01..14 Row-by-Row Positive Capability Reconciliation

Every capability from `docs/program/S2_CAPABILITY_MATRIX.md` is adjudicated below using exact literal test function names:
...
|---|---|---|---|---|---|---|
| **S2-AC-01** | Dataset / capture input-boundary validation | REQUIRED | `ReplayInputBoundary` / `RunInputBoundary` (`replay/core.py:160-271`) binds run ID, code revision, config hash, provider, capture scope, ordered event IDs, content hashes, and temporal semantics. `normalize_market_batch` (`data_platform/normalization.py:85-149`) enforces explicit lane parameters. | `test_replay_input_boundary_initialization_and_validation`, `test_replay_input_boundary_factories`, `test_schedule_boundary_validation_fail_closed`, `test_explicit_boundary_enforcement_regressions`, `test_normalize_market_batch_parameter_conflicts` | PR #65 (`2005310`), PR #67 (`5c2c94b`) | **PASS** |
| **S2-AC-02** | Lossless market-data normalization preserving source facts and missingness | REQUIRED | `normalize_market_batch` and `NormalizedMarketBatch` (`data_platform/normalization.py:14-149`) preserve all envelope attributes, timestamps, payloads, and `MissingReason` without modification or loss. The caller iterable is defensively materialized into an immutable tuple while envelopes are preserved by identity. | `test_exact_preservation_of_source_facts`, `test_tick_missingness_preservation_individual`, `test_tick_multiple_missing_fields_preserved`, `test_candle_missingness_preservation`, `test_source_envelope_remains_unchanged` | PR #67 (`850ca2e`, `5c2c94b`) | **PASS** |
| **S2-AC-03** | Provider/capture-scope causal lane partitioning | REQUIRED | `CausalLane` (`replay/core.py:44-54`) partitions events by `(provider_id, capture_scope)`. Both `CausalMarketReplaySchedule` and `NormalizedMarketBatch` reject mixed providers or mixed capture scopes fail-closed. | `test_mixed_provider_rejection`, `test_mixed_capture_scope_rejection`, `test_multiple_instruments_in_same_lane` (in `test_causal_replay_core.py` and `test_normalization_quality.py`) | PR #65 (`6985fdf`), PR #67 (`850ca2e`) | **PASS** |
| **S2-AC-04** | Immutable replay schedule | REQUIRED | `CausalMarketReplaySchedule` (`replay/core.py:274-458`) is a frozen dataclass with slots storing immutable tuples of events. The caller iterable is defensively materialized into an immutable tuple. The accepted immutable `EventEnvelope` instances are preserved by identity and are neither mutated nor reconstructed; caller-supplied order is preserved. | `test_defensive_input_capture_and_immutability`, `test_equal_knowledge_times_preserve_supplied_order`, `test_knowledge_time_regression_rejected_without_sorting` | PR #65 (`6985fdf`, `2005310`) | **PASS** |
| **S2-AC-05** | Explicit inclusive knowledge-cutoff advancement | REQUIRED | `CausalMarketReplayCursor.advance_to()` (`replay/core.py:493-528`) emits all events where `knowledge_time <= knowledge_cutoff`. Does not emit events beyond the cutoff. | `test_cutoff_before_first_event`, `test_cutoff_exactly_at_event`, `test_cutoff_after_multiple_events`, `test_repeated_same_cutoff_behavior` | PR #65 (`6985fdf`) | **PASS** |
| **S2-AC-06** | Controllable virtual clock with monotonic advancement | REQUIRED | `CausalMarketReplayCursor` advances virtual time monotonically. Backward cutoff movement raises `ValueError` fail-closed without mutating cursor state or advancing index. | `test_backward_cutoff_rollback_safety`, `test_missing_unknown_or_naive_knowledge_time_rejection` | PR #65 (`6985fdf`) | **PASS** |
| **S2-AC-07** | Exact logical replay-speed representation without wall-clock dependence | REQUIRED | `ReplaySpeed` (`replay/core.py:56-93`) represents speed as exact positive rational numbers (`Fraction`). Virtual delays are calculated with exact arithmetic without `time.sleep` or wall-clock dependencies. | `test_exact_speed_1x`, `test_exact_speed_2x`, `test_exact_speed_half_x`, `test_arbitrary_positive_rational_speed`, `test_zero_speed_rejection`, `test_negative_speed_rejection`, `test_static_absence_of_wall_clock_network_and_financial_capability` | PR #65 (`6985fdf`) | **PASS** |
| **S2-AC-08** | Replay input provenance / manifest | REQUIRED | `ReplayInputBoundary` (`replay/core.py:160-271`) binds source identities, content hashes, code revision, and configuration hash. Supports factory instantiation via `from_manifest` and `from_events`. | `test_replay_input_boundary_initialization_and_validation`, `test_replay_input_boundary_partial_content_hashes_and_input_identities`, `test_replay_input_boundary_factories`, `test_schedule_boundary_validation_fail_closed` | PR #65 (`2005310`) | **PASS** |
| **S2-AC-09** | Replay run identity and lineage from input evidence to outputs | REQUIRED | `ReplayEmissionLineage` and `ReplayEmission.lineage` (`replay/core.py:95-115, 149-157`) bind emitted records to `run_id`, source lane, ordinal, and `event_id`. `QualityFinding` binds findings to source `event_id`. | `test_replay_emission_lineage_and_properties`, `test_schedule_lineage_preservation_during_replay`, `test_finding_to_source_event_id_lineage`, `test_normalized_market_batch_finding_lineage_closure` | PR #65 (`2005310`), PR #67 (`5c2c94b`) | **PASS** |
| **S2-AC-10** | Data-quality evidence and fail-closed anomaly reporting | REQUIRED | `QualityFinding` and `evaluate_envelope_quality` (`data_platform/quality.py:52-241`) provide factual anomaly evidence; `NormalizedMarketBatch.blocks_replay` flags replay-blocking anomalies. | `test_missing_or_unknown_knowledge_time_marked_replay_blocking`, `test_known_knowledge_time_retained_unmutated`, `test_missing_event_time_evidence_surfaced_as_quality_evidence`, `test_clean_candle_produces_no_findings` | PR #67 (`850ca2e`, `5c2c94b`) | **PASS** |
...
| **S2-AC-12** | Deterministic repeated replay of identical logical inputs | REQUIRED | Pure deterministic execution: identical inputs, code revision, and configuration produce identical sequences of `ReplayEmission` and `NormalizedMarketBatch` records. | `test_deterministic_repeated_schedule_construction_and_replay` (`test_causal_replay_core.py`), `test_deterministic_repeated_normalization` (`test_normalization_quality.py`) | PR #65 (`6985fdf`), PR #67 (`850ca2e`) | **PASS** |
...
Every prohibition from `docs/program/S2_CAPABILITY_MATRIX.md` and `docs/program/S2_ENTRY_CONTRACT.md` is audited below using exact literal test function names:

| ID | Prohibition | Enforcement Mechanism | Evidence of Absence | Verdict |
|---|---|---|---|---|
| **S2-NC-01** | Strategy operational path | `scripts/check_s2_boundary.py` AST/name scanner + code review | Prohibited identifiers (`StrategyDecision`, `StrategyEngine`) absent from S2 roots. Verified by `test_allowed_python_file_passes`, `test_financial_and_temporal_prohibitions_fail`, `test_repo_s2_roots_pass_boundary` in `tests/test_s2_boundary.py`. | **PASS** |
| **S2-NC-02** | Signal/trade intent operational path | Boundary scanner + AST verification | Prohibited identifiers (`TradeIntent`, `SignalGenerator`) absent from S2 roots. Verified by `test_financial_and_temporal_prohibitions_fail`, `test_repo_s2_roots_pass_boundary` in `tests/test_s2_boundary.py`. | **PASS** |
| **S2-NC-03** | Risk authorization path | Boundary scanner + AST verification | Prohibited identifiers (`RiskAuthorization`, `RiskEngine`) absent from S2 roots. Verified by `test_financial_and_temporal_prohibitions_fail`, `test_repo_s2_roots_pass_boundary` in `tests/test_s2_boundary.py`. | **PASS** |
| **S2-NC-04** | Paper execution | Boundary scanner + AST verification | Prohibited identifier (`PaperExecution`) absent from S2 roots. Verified by `test_financial_and_temporal_prohibitions_fail`, `test_repo_s2_roots_pass_boundary` in `tests/test_s2_boundary.py`. | **PASS** |
| **S2-NC-05** | Live execution | Boundary scanner + AST verification | Prohibited identifiers (`ExecutionEngine`, `ExecutionOrder`) absent from S2 roots. Verified by `test_financial_and_temporal_prohibitions_fail`, `test_repo_s2_roots_pass_boundary` in `tests/test_s2_boundary.py`. | **PASS** |
| **S2-NC-06** | Broker order/account API | Boundary scanner + import inspection | Prohibited imports (`MetaTrader5`) and calls (`order_send`, `order_check`, `account_info`, `positions_get`) absent. Verified by `test_allowed_python_file_passes`, `test_non_python_file_with_secret_fails`, `test_non_python_named_credential_literal_fails`, `test_python_named_credential_literal_fails`, `test_financial_and_temporal_prohibitions_fail`, `test_repo_s2_roots_pass_boundary` in `tests/test_s2_boundary.py`. | **PASS** |
| **S2-NC-07** | Financial Ledger mutation | Boundary scanner + AST verification | Prohibited identifiers (`FinancialLedger`, `PositionTracker`) absent from S2 roots. Verified by `test_financial_and_temporal_prohibitions_fail`, `test_repo_s2_roots_pass_boundary` in `tests/test_s2_boundary.py`. | **PASS** |
| **S2-NC-08** | P&L / economic backtest | Boundary scanner + economic name check | Prohibited identifiers (`PnL`, `ProfitAndLoss`, `EconomicBacktest`) absent from S2 roots. Verified by `test_financial_and_temporal_prohibitions_fail`, `test_repo_s2_roots_pass_boundary` in `tests/test_s2_boundary.py`. | **PASS** |
| **S2-NC-09** | Spread/fees/slippage/queue-fill economics | Boundary scanner + economic name check | Prohibited identifiers (`SlippageModel`, `FeeModel`, `SpreadModel`, `FillModel`, `QueueFillModel`) absent from S2 roots. Verified by `test_financial_and_temporal_prohibitions_fail`, `test_repo_s2_roots_pass_boundary` in `tests/test_s2_boundary.py`. | **PASS** |
| **S2-NC-10** | Predictive ML operational wiring | Boundary scanner + import scanner | No machine learning frameworks, model training, feature stores, or prediction pipelines wired in S2 runtime. Verified by `test_static_absence_of_wall_clock_network_and_financial_capability` (`tests/replay/test_causal_replay_core.py`), `test_static_absence_of_prohibited_capabilities` (`tests/data_platform/test_normalization_quality.py`), and `test_repo_s2_roots_pass_boundary` (`tests/test_s2_boundary.py`). | **PASS** |
| **S2-NC-11** | Real-money authority | Architectural design + lack of financial pathways | Zero financial authority granted, zero execution pathways, zero broker accounts connected. Verified by `test_static_absence_of_wall_clock_network_and_financial_capability`, `test_static_absence_of_prohibited_capabilities`, and `test_repo_s2_roots_pass_boundary`. | **PASS** |
| **S2-NC-12** | Mutation of source evidence | Immutable dataclasses + defensive materialization | The caller iterable is defensively materialized into an immutable tuple. The accepted immutable `EventEnvelope` instances are preserved by identity and are neither mutated nor reconstructed. Verified by `test_defensive_input_capture_and_immutability` (`tests/replay/test_causal_replay_core.py`), `test_defensive_input_capture_and_immutability` and `test_source_envelope_remains_unchanged` (`tests/data_platform/test_normalization_quality.py`). | **PASS** |
| **S2-NC-13** | Fabricated temporal evidence | Type safety + fail-closed validation | Replay rejects missing/naive knowledge time fail-closed; normalization surfaces explicit quality findings without deriving or synthesizing timestamps. Verified by `test_missing_unknown_or_naive_knowledge_time_rejection` (`tests/replay/test_causal_replay_core.py`), `test_missing_or_unknown_knowledge_time_marked_replay_blocking` and `test_no_temporal_axis_substitution` (`tests/data_platform/test_normalization_quality.py`). | **PASS** |
| **S2-NC-14** | Synthetic cross-lane ordering | Lane isolation + fail-closed validation | Replay and normalization fail closed on mixed provider or capture scope; no synthetic interleaving or global sorting is performed. Verified by `test_mixed_provider_rejection` and `test_mixed_capture_scope_rejection` in `tests/replay/test_causal_replay_core.py` and `tests/data_platform/test_normalization_quality.py`. | **PASS** |
| **S2-NC-15** | Silent missing-data imputation | Lossless normalization + missingness tests | `MissingReason` preserved across all fields; zero forward fill, backward fill, interpolation, or zero-filling. Verified by `test_tick_missingness_preservation_individual`, `test_tick_multiple_missing_fields_preserved`, `test_candle_missingness_preservation` in `tests/data_platform/test_normalization_quality.py`. | **PASS** |
| **S2-NC-16** | Wall-clock causal semantics | Boundary scanner + pure virtual pacing | `time.sleep`, `asyncio.sleep`, `datetime.now`, `time.time` forbidden and checked by `scripts/check_s2_boundary.py`. Replay pacing is purely logical using exact `Fraction`. Verified by `test_static_absence_of_wall_clock_network_and_financial_capability` in `tests/replay/test_causal_replay_core.py`, `test_static_absence_of_prohibited_capabilities` in `tests/data_platform/test_normalization_quality.py`, and `test_financial_and_temporal_prohibitions_fail` in `tests/test_s2_boundary.py`. | **PASS** |
...
| **#61** | Administrative Branch Protection & Ruleset Hardening | **NON_BLOCKING** | Tracks administrative configuration of GitHub branch protection rulesets. Administrative branch/ruleset protection remains absent/pending because the automation environment lacks administrative permissions to configure rulesets via API; Issue #61 remains open as defense-in-depth. CI, boundary scanners, and manual pull request reviews provide technical verification gates, but these mechanisms do NOT replace GitHub administrative enforcement. Under the governing Sprint 2 Entry Contract, administrative protection is not a functional closure requirement. Therefore, Issue #61 is non-blocking for Sprint 2 closure. |
...
The closure pull request (#70) must execute and pass the full suite of automated checks on its exact HEAD commit prior to merge:
