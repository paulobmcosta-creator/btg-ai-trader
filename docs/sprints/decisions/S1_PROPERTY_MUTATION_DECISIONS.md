...
Mutants target actual invariant behavior: FIFO delivery, observed rejection counters, duplicate payload conflict classification, capacity exhaustion classification, canonical codec representation rejection, raw-hash consistency, duplicate JSON-key rejection, equality at the late frontier, lineage-cycle rejection and lineage-hash consistency. Mutations do not edit canonical source. Only a child test failure in an executed selected property group counts as killed; import/collection errors, timeouts and invalid harness setup are errors, not kills.
